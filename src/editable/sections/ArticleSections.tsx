    ...
    <main className={dc.shell.page}>
      <section className={`${dc.shell.section} pt-12 sm:pt-16 lg:pt-20`}>
        <div className={`rounded-[2.5rem] border ${pal.border} ${pal.darkBg} p-7 text-white shadow-[0_24px_80px_rgba(24,20,17,0.18)] sm:p-10 lg:p-14`}>
          ...
          <form action={basePath} className="mt-8 flex max-w-xl flex-col gap-3 sm:flex-row">
            <select name="category" defaultValue={category || 'all'} className={`min-w-0 flex-1 rounded-full border ${pal.darkBorder} border-white/10 bg-white/[0.04] px-5 py-3 text-sm font-bold text-white outline-none`}>
      ...
      <section className={`${dc.shell.section} ${dc.shell.sectionY}`}>
        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
          {pagination.hasPrevPage ? <Link href={pageHref(page - 1)} className={`rounded-full border border-white/10 px-5 py-3 text-sm font-bold text-white/60`}>Previous</Link> : null}
          <span className={`rounded-full ${pal.darkBg} px-5 py-3 text-sm font-black text-white`}>Page {page} of {pagination.totalPages || 1}</span>
          {pagination.hasNextPage ? <Link href={pageHref(page + 1)} className={`rounded-full border border-white/10 px-5 py-3 text-sm font-bold text-white/60`}>Next</Link> : null}
        </div>
      </section>
    </main>
  )
  ...
  return (
    <main className={dc.shell.page}>
      <section className={`${dc.shell.section} pt-10 sm:pt-14 lg:pt-16`}>
        <div className={`grid gap-6 rounded-2xl border ${pal.border} ${pal.surfaceBg} p-6 lg:grid-cols-[minmax(0,1fr)_320px] lg:p-10`}>
          ...
          <aside className={`min-w-0 rounded-[2rem] ${pal.darkBg} p-6 text-white`}>
            ...
            <Link href="/contact" className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#c8ff00] px-5 py-3 text-sm font-bold text-[#0b0b0f]">Contact <ArrowRight className="h-4 w-4" /></Link>
          </aside>
        </div>
      </section>
      <section className="mx-auto w-full max-w-5xl px-4 pb-16 pt-6 sm:px-6 lg:px-8 lg:pb-24">
        <div className={`rounded-2xl border ${pal.border} ${pal.surfaceBg} p-6 sm:p-8 lg:p-10`}>
